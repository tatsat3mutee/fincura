...
All endpoints are read-only and user-scoped.
"""
...
def _prev_months(n: int) -> list[str]:
    """Return the last n complete calendar months as YYYY-MM strings."""
    today = date.today()
...
async def anomalies(current_user=Depends(get_current_user)):
    ...
    Returns list of {category, icon, avg_3m, current_month, ratio}.
    """
    ...
    async with db.get_db() as conn:
        ...
        current = {row["category"]: {"icon": row["icon"], "total": row["total"]} for row in current_rows}
...
async def goal_projections(current_user=Depends(get_current_user)):
    """
    For each active goal, estimate completion date based on monthly savings rate (Savings category).
    """
    user_id = current_user["id"]
    today = date.today()
    prev_months = _prev_months(3)
    ...
    async with db.get_db() as conn:
        ...
        # Monthly savings rate = total expense in Savings category over last 3 months / 3
        cur2 = await conn.execute(
...
async def anomalies(current_user=Depends(get_current_user)):
    """
    Flag categories where this month's spend is >150% of the 3-month rolling average.
    Returns a list of {category, avg_3m, current_month, ratio} objects.
    """
    user_id = current_user["id"]
    today = date.today()
    this_month = today.strftime("%Y-%m")
    ...
    for _ in range(3):
        ...
        if month == 0:
            month = 12
            year -= 1
        prev_months.append(f"{year:04d}-{month:02d}")
    ...
    async with db.get_db() as conn:
        ...
        current = {row["category"]: {"icon": row["icon"], "total": row["total"]} async for row in cur}
        ...
        averages = {row["category"]: row["avg_3m"] async for row in cur2}
...
async def goal_projections(current_user=Depends(get_current_user)):
    """
    For each active savings goal, estimate completion date based on monthly deposit rate.
    """
    user_id = current_user["id"]
    async with db.get_db() as conn:
        cur = await conn.execute(
        ...
        goals = [dict(row) async for row in cur]
    ...
    for g in goals:
        ...
        # Monthly deposit rate: average over last 3 months
        months_back = []
        year, month = today.year, today.month
        for _ in range(3):
            month -= 1
            if month == 0:
                month = 12
                year -= 1
            months_back.append(f"{year:04d}-{month:02d}")

        placeholders = ",".join("?" * len(months_back))
        async with db.get_db() as conn:
            cur2 = await conn.execute(
                f"SELECT AVG(monthly) as avg_deposit FROM ("
                f"  SELECT strftime('%Y-%m', txn_date) as mo, SUM(amount) as monthly"
                f"  FROM transactions"
                f"  WHERE user_id = ? AND savings_goal_id = ?"
                f"    AND strftime('%Y-%m', txn_date) IN ({placeholders})"
                f"  GROUP BY mo"
                f") sub",
                (user_id, g["id"], *months_back),
            )
            row = await cur2.fetchone()
            monthly_rate = row["avg_deposit"] if row and row["avg_deposit"] else None
        ...
        if not monthly_rate or monthly_rate <= 0:
            ...
            est_year = today.year + (today.month - 1 + int(months_needed)) // 12
            est_month = (today.month - 1 + int(months_needed)) % 12 + 1
            projections.append({
                "id": g["id"], "name": g["name"],
                "remaining": round(remaining, 2),
